...
class ClickResultService(BaseService):
    ...
    async def get_clicks_stats(
        ...
        data = await self.repository.get_clicks_data(
            session=session, copyname=copyname, period=period, ask=ask
        )
        main_data = []
        ...
        main_df_grouped, main_latest_pos = await self.grouping_stats_data(
            main_df, grouping=grouping
        )

        if (df_grouped is not None and not df_grouped.empty) or (
            main_df_grouped is not None and not main_df_grouped.empty
        ):
            return {
                "df_grouped": df_grouped,
                "main_df_grouped": main_df_grouped,
                "latest_pos": latest_pos,
                "main_latest_pos": main_latest_pos,
                "addings_count": addings_count,
    ...
    async def new_create_graphics(
        self,
        latest_pos,
        addings_count,
        total_count,
        df_grouped: pd.DataFrame = pd.DataFrame(),
        main_df_grouped: pd.DataFrame = pd.DataFrame(),
        ask=None,
    ):
        ...
        max_pos = (
            max(np.ceil(df_grouped["pos"].max()), 10) if not df_grouped.empty else 10
        )
        plt.ylim(
            max_pos, 0.5
        )  # Устанавливаем диапазон от max_pos (снизу) до 0.5 (сверху)
        ...
        if latest_pos is not None and (
            not df_grouped.empty or not main_df_grouped.empty
        ):
            if df_grouped.empty:
