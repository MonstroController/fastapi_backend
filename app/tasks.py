...
def fetch_profiles(existing):
    ...
    """
    interval_hours = settings.profiles.TIME_BEFORE_DATE_BLOCK
    cur.execute(query, (settings.profiles.WORKING_PARTY, max_date, min_date, interval_hours))
    ...
    if len(parties) != 0:
        ...
        if party_fraction > 0:
            for party, _ in parties:
                ...
                total += len(res)
                profiles.extend(res)
        logger.info(f"party_fraction: {party_fraction}, total: {total}, count: {count}, parties: {len(parties)}")
...
def delete_profiles_by_pid(pids, conn):
    ...
    cur.execute(stats_query, ("transport_1", len(pids),))
    conn.commit()
    cur.close()
    conn.close()
...
def transfer_profiles():
    ...
    if inserted_pids:
        ...
        logger.info(f"Перемещено {len(inserted_pids)} профилей. Всего за день: {current_count + len(inserted_pids)}")
